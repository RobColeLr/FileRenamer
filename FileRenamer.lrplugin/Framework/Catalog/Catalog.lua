...
function Catalog:getFolder( photo, photoPath )
    ...
    return self:getFolderByPath( LrPathUtils.parent( photoPath ) ) -- ###1 bypass cache?
end
...
function Catalog:getFolderSet( photos, cache )
    ...
    for i, photo in ipairs( photos ) do
        ...
        local folder = cat:getFolderByPath( fpath ) -- method changed 27/May/2014 23:03 ###1
        if folder then
...
function Catalog:_update( catFunc, tmo, name, func, ... )
    ...
    tmo = tmo or 0
    --local tmoTbl = { timeout = tmo } -- ###1 until 1/Jun/2014 23:42, but note: in one case, tmo is negative 5 (relative antics), to mean "don't prompt if no go" (for background tasks).
...
function Catalog:_assureSources( photo, photos, metaCache )
    ...
    for i, __photo in ipairs( photos ) do
        repeat
            ...
            local lrFolder = cat:getFolderByPath( parent ) -- method change unreleased ###1
            if lrFolder then
...
function Catalog:assurePhotoIsSelected( photo, photoPath )
    ...
    if photo then
        ...
        if catalog:getTargetPhoto() ~= photo then -- photo not selected (not visible or not in filmstrip).
            ...
            local lrFolder = cat:getFolderByPath( folderPath ) -- method change unreleased ###1
            local found = false
            if lrFolder then
    ...
    if filter.columnBrowserActive then -- metadata filtering enabled.
        ...
        for _, item in ipairs( metaIds ) do
            ...
            for i, t in ipairs( filter.columnBrowserDesc ) do
                ...
                if name == item then
                    if t.criteria:find( _PLUGIN.id ) then -- ###1 should probably be plain text search, not pattern.
