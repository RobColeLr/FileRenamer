...
#
    ...
    if ($valStart < $len) {
        ...
        if ($minPtr) {
            ...
            if (not defined $expect) {
                $exifTool->Warn('Unrecognized Leica trailer structure');
    ...
    if ($newPos) { # are we writing?
        if ($$exifTool{Model} ne 'S2') {
            $exifTool->Warn('Leica MakerNote trailer too messed up to edit.  Copying as a block', 1);
            return $buff;
        }
